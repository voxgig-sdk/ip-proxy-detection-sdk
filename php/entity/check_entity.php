...
// IpProxyDetection SDK Check entity
...
class CheckEntity
{
    ...
    {
        $opts = $this->_entopts;
        return new CheckEntity($this->_client, $opts);
    }

    /**
     * @param Check|array $args Check data (assoc-array) to store.
     */
    public function data_set($args): void
    {
        if ($args) {
            $this->_data = IpProxyDetectionHelpers::to_map(Struct::clone($args)) ?? [];
            ($this->_utility->feature_hook)($this->_entctx, "SetData");
        }
    }

    /**
     * @return Check|array The current Check data as an assoc-array.
     */
    ...
    /**
     * @param array $args Match filter (any subset of Check fields).
     */
    public function match_set($args): void
    {
        if ($args) {
            $this->_match = IpProxyDetectionHelpers::to_map(Struct::clone($args)) ?? [];
            ($this->_utility->feature_hook)($this->_entctx, "SetMatch");
        }
    }

    /**
     * @return array The current match filter (any subset of Check fields).
     */
     ...
     * Load a single Check.
     *
     * @param CheckLoadMatch|array|null $reqmatch Match criteria (id/query
     *   fields) as an assoc-array; a typed CheckLoadMatch names the shape.
     * @param mixed $ctrl Optional per-call control overrides.
     * @return Check|array The loaded Check as an assoc-array at the
     *   SDK boundary; throws IpProxyDetectionError on failure (item-5 convention).
     */
        ...
        return $this->_run_op($ctx, function () use ($ctx) {
            if ($ctx->result) {
                ...
                if ($ctx->result->resdata) {
                    $this->_data = IpProxyDetectionHelpers::to_map(Struct::clone($ctx->result->resdata)) ?? [];
                }
            }
        });
    }
