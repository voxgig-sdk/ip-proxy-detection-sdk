...
// IpProxyDetection SDK feature test
//
...
class FtCtrl extends IpProxyDetectionControl
{
...
class FtClient
{
    ...
    private IpProxyDetectionUtility $utility;

    public function __construct(IpProxyDetectionUtility $utility, array $options)
    {
    ...
    public function get_utility(): IpProxyDetectionUtility
    {
        return $this->utility;
    }
}
...
class FtRecorder
{
    ...
    {
        return function (IpProxyDetectionContext $_ctx, string $url, array $fetchdef): array {
...
class FtHarness
{
    ...
    public IpProxyDetectionContext $rootctx;
    ...
    public function __construct(
        ...
        $utility->fetcher = $server ?? function (IpProxyDetectionContext $_ctx, string $_url, array $fetchdef): array {
        ...
        foreach ($features as $fspec) {
            ...
            $f = IpProxyDetectionConfig::make_feature($name);
    ...
    private static function buildUrl(IpProxyDetectionSpec $spec): string
    {
        $keys = [];
    ...
    public function op(array $o = []): array
    {
        ...
        if (($ctx->out['point'] ?? null) instanceof IpProxyDetectionError) {
        ...
        if (is_array($fetched)) {
            $ctx->response = new IpProxyDetectionResponse($fetched);
        }
    ...
    private function fail(IpProxyDetectionContext $ctx, mixed $err): array
    {
...
class FeatureTest extends TestCase
{
    ...
    public function test_retry_retries_a_transport_error_until_exhausted(): void
    {
        ...
        $clock = new FtClock();
        $n = 0;
        $server = function (IpProxyDetectionContext $ctx, string $_url, array $_fd) use (&$n): array {
            $n++;
