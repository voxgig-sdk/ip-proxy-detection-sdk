...
//    only - a typed IpProxyDetectionContext there would make every ctx
...
final class IpProxyDetectionOmni
{
    ...
    public static function ctx_from_map($ctxmap, $client, $utility)
    {
        ...
        $ctx = new IpProxyDetectionContext($clean, null);
        ...
        if (isset($clean['spec']) && is_array($clean['spec'])) {
            $ctx->spec = new IpProxyDetectionSpec($clean['spec']);
        }

        if (isset($clean['result']) && is_array($clean['result'])) {
            $ctx->result = new IpProxyDetectionResult($clean['result']);
            if (isset($clean['result']['err']) && is_array($clean['result']['err'])) {
                $msg = $clean['result']['err']['message'] ?? '';
                $ctx->result->err = new IpProxyDetectionError('', is_string($msg) ? $msg : '');
            }
        }

        if (isset($clean['response']) && is_array($clean['response'])) {
            $ctx->response = new IpProxyDetectionResponse($clean['response']);
    ...
    public static function err_from_map($m): ?IpProxyDetectionError
    {
        if (!is_array($m)) {
            return null;
        }
        ...
        $code = $m['code'] ?? '';
        return new IpProxyDetectionError(is_string($code) ? $code : '', $msg);
    }
