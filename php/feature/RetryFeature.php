...
// IpProxyDetection SDK retry feature
...
class IpProxyDetectionRetryFeature extends IpProxyDetectionBaseFeature
{
    ...
    public function init(IpProxyDetectionContext $ctx, array $options): void
    {
        ...
        $utility->fetcher = function (IpProxyDetectionContext $ctx2, string $url, array $fetchdef) use ($inner): array {
            return $this->_with_retry($ctx2, $url, $fetchdef, $inner);
        };
    }

    private function _with_retry(IpProxyDetectionContext $ctx, string $url, array $fetchdef, callable $inner): array
    {
        $opts = $this->options ?? [];
    ...
    private function _track(IpProxyDetectionContext $_ctx, int $attempt, mixed $res, mixed $err, float $wait): void
    {
        $client = $this->client;
