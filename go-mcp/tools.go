...
type Args struct {
	Entity string         `json:"entity" jsonschema:"check"`
	Query  map[string]any `json:"query,omitempty" jsonschema:"optional match map e.g. {\"id\":1} for load, omit for list"`
}

func registerTools(server *mcp.Server, client *sdk.IpProxyDetectionSDK) {
	mcp.AddTool(server, &mcp.Tool{
		Name: "ip-proxy-detection_list",
		Description: "List records from IpProxyDetection. " +
	...
	mcp.AddTool(server, &mcp.Tool{
		Name: "ip-proxy-detection_load",
		Description: "Load a single record from IpProxyDetection. " +
...
func runOp(client *sdk.IpProxyDetectionSDK, op string, args Args) (*mcp.CallToolResult, any, error) {
...
func entityFor(client *sdk.IpProxyDetectionSDK, name string) (sdk.IpProxyDetectionEntity, error) {
