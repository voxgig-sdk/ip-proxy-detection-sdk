# ip-proxy-detection-mcp

[MCP](https://modelcontextprotocol.io) server exposing the IpProxyDetection SDK as
two agent tools — `ip-proxy-detection_list` and `ip-proxy-detection_load` — built on the
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/ip-proxy-detection-mcp)
make build

# 2. Provide credentials via the environment
export IP_PROXY_DETECTION_APIKEY=sk_live_xxx

# 3a. Install into Claude Code over stdio (most common)
claude mcp add --scope user ip-proxy-detection \
  -- /absolute/path/to/ip-proxy-detection-mcp -transport stdio

# 3b. …or run a shared HTTP server instead
./ip-proxy-detection-mcp -transport http -addr :8080
```

Tool-call arguments (what an agent sends):

```jsonc
// ip-proxy-detection_list: first page of records
{ "entity": "check" }
{ "entity": "check", "query": { } }

// ip-proxy-detection_load: one record by id
{ "entity": "check", "query": { "id": 1 } }
```
...
1. **Build** the server from this `go-mcp/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/ip-proxy-detection-mcp
   ```
...
3. **Install it into Claude Code** (stdio transport):
   ...
   ```sh
   claude mcp add --scope user ip-proxy-detection \
     -- "$PWD"/dist/*/ip-proxy-detection-mcp -transport stdio
   ```

4. **Restart Claude Code.** The `ip-proxy-detection_list` and `ip-proxy-detection_load` tools now appear
   in new sessions. Ask the agent to *"list check using ip-proxy-detection"*
   and it calls `ip-proxy-detection_list` with `{"entity":"check"}`.
...
```sh
export IP_PROXY_DETECTION_APIKEY=sk_live_xxx            # API key
export IP_PROXY_DETECTION_BASE=https://api.example.com  # optional: override the API base URL
```
...
### Call the `ip-proxy-detection_list` tool
...
### Call the `ip-proxy-detection_load` tool
...
| Tool | Args | Returns |
|------|------|---------|
| `ip-proxy-detection_list` | `entity` (required), `query` (optional map) | First page of records as JSON |
| `ip-proxy-detection_load` | `entity` (required), `query` = `{id:N}` | Single record as JSON |
...
| Variable | Purpose |
|----------|---------|
| `IP_PROXY_DETECTION_APIKEY` | API key sent with every request. |
| `IP_PROXY_DETECTION_BASE` | Optional override of the API base URL. |
...
```sh
./ip-proxy-detection-mcp -transport http -addr :18080 &
...
curl -sN -X POST http://localhost:18080 \
  ...
  -d '{"jsonrpc":"2.0","id":2,"method":"tools/call","params":{"name":"ip-proxy-detection_load","arguments":{"entity":"check","query":{"id":1}}}}'
```
