...
//     (\`new IpProxyDetectionSDK(...)\` / \`IpProxyDetectionSDK.test(...)\` become
//     \`IpProxyDetectionSDK.test({ entity: ... })\`), transpiled to JS, and run
...
const SDK_NAME = 'IpProxyDetectionSDK'
...
const TEST_SEED = {"entity":{"check":{"test01":{"id":"test01"}}}}
...
// rewrite real client construction (\`new IpProxyDetectionSDK(...)\` and any
// \`IpProxyDetectionSDK.test(...)\`) to a seeded test-mode client, and give
...
function rewriteForRun(code: string): string {
  ...
  out = out.replace(/new\s+IpProxyDetectionSDK\s*\([^)]*\)/g, () => SEEDED_CTOR)
  out = out.replace(/IpProxyDetectionSDK\.test\s*\([^)]*\)/g, () => SEEDED_CTOR)
