...
describe('CheckDirect', async () => {
  ...
  test('direct-exists', async () => {
    const sdk = new IpProxyDetectionSDK({
  ...
  test('direct-load-check', async (t: any) => {
    if (liveScenariosActive()) { t.skip('Covered by live operation scenarios'); return }
    const setup = directSetup({ id: 'direct01' })
    if (maybeSkipControl(t, 'direct', 'direct-load-check', setup.live)) return
...
function directSetup(mockres?: any) {
  ...
  if (live) {
    ...
    let idmap: any = env['IP_PROXY_DETECTION_TEST_CHECK_ENTID']
