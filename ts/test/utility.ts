...
function envOverride(m: Record<string, any>) {
  if (
    'TRUE' === process.env.IPPROXYDETECTION_TEST_LIVE ||
    'TRUE' === process.env.IPPROXYDETECTION_TEST_OVERRIDE
  ) {
  ...
  m.IPPROXYDETECTION_TEST_EXPLAIN = process.env.IPPROXYDETECTION_TEST_EXPLAIN || m.IPPROXYDETECTION_TEST_EXPLAIN
