...
boru-driven command-line client **and** interactive REPL for the IpProxyDetection
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/ip-proxy-detection-cli)
make build

# 2. See usage (words, entities, env vars)
./ip-proxy-detection-cli --help

# 3. Provide credentials once, via the environment
export IP_PROXY_DETECTION_APIKEY=sk_live_xxx

# 4. Each command line is ONE boru expression, run against the API:
./ip-proxy-detection-cli load 1 check            # {id:1} shorthand
./ip-proxy-detection-cli load '{id:1}' check       # explicit match map

# 5. Override the API base URL for a single call
IP_PROXY_DETECTION_BASE=https://api.example.com ./ip-proxy-detection-cli load 1 check

# 6. No arguments -> interactive REPL
./ip-proxy-detection-cli
ip-proxy-detection> load 1 check
ip-proxy-detection> /quit
```
...
1. **Build the binary.** From this `go-cli/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/ip-proxy-detection-cli
   ```
...
3. **Run a query.** Evaluate an boru expression against the API (or run with no
   ...
   ```sh
   ./dist/*/ip-proxy-detection-cli load 1 check
   ```
...
```sh
./ip-proxy-detection-cli load 1 check          # scalar shorthand for {id:1}
./ip-proxy-detection-cli load '{id:1}' check     # explicit match map
```
...
Run with no arguments to open a REPL (prompt `ip-proxy-detection>`). Each line is
evaluated as its own boru expression:

```text
$ ./ip-proxy-detection-cli
ip-proxy-detection> load 1 check
ip-proxy-detection> /help
ip-proxy-detection> /quit
```
...
- `<entity>` is a bareword, auto-quoted as an boru atom (e.g. `check`).
...
| Target | Result |
|--------|--------|
| `make build` | Native binary at `dist/<os>-<arch>/ip-proxy-detection-cli`. |
...
(`./ip-proxy-detection-cli <expr>`) and interactively (the REPL), and expressions compose the
