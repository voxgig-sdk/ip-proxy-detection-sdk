...
local CheckEntity = {}
CheckEntity.__index = CheckEntity


function CheckEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, CheckEntity)
  self._name = "check"
  self._client = client
...
function CheckEntity:get_name()
  return self._name
end


function CheckEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return CheckEntity.new(self._client, opts)
end
...
function CheckEntity:mark_deleted()
  self._deleted = true
end


function CheckEntity:deleted()
  return true == self._deleted
end


function CheckEntity:data_set(args)
  if args ~= nil then
...
function CheckEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function CheckEntity:match_set(args)
  if args ~= nil then
...
function CheckEntity:match_get()
...
function CheckEntity:stream(action, args, callopts)
...
---@param reqmatch CheckLoadMatch
---@param ctrl? table
---@return Check
---@return string? err
function CheckEntity:load(reqmatch, ctrl)
  local utility = self._utility
...
function CheckEntity:_run_op(ctx, post_done)
  local utility = self._utility
