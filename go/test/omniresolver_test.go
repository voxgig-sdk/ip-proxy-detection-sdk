...
import (
	...
	omni "github.com/voxgig-sdk/ip-proxy-detection-sdk/go/test/omni"
...
func omniProvider(client any) *omni.Provider {
	provider := &omni.Provider{
		...
		Errify: func(err any) any {
			if sdkerr, is := err.(*sdk.IpProxyDetectionError); is {
				out := map[string]any{
					"name":    "IpProxyDetectionError",
...
func omniCtx(arg any, client *sdk.IpProxyDetectionSDK, utility *sdk.Utility) *sdk.Context {
	...
	if provider, is := ctxmap["client"].(*omni.Provider); is {
		if live, is := defproviderclient(provider).(*sdk.IpProxyDetectionSDK); is {
			client = live
