...
func TestCheckEntity(t *testing.T) {
	...
	t.Run("basic", func(t *testing.T) {
		...
		checkRef01DataRaw := vs.Items(core.ToMapAny(vs.GetPath(setup.data, "existing.check")))
...
func checkBasicSetup(extra map[string]any) *entityTestSetup {
	...
	entidEnvRaw := os.Getenv("IP_PROXY_DETECTION_TEST_CHECK_ENTID")
	...
	idmapResolved := core.ToMapAny(env["IP_PROXY_DETECTION_TEST_CHECK_ENTID"])
