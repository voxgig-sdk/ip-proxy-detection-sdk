# IpProxyDetection SDK test runner
...
class IpProxyDetectionTestRunner:
    _env = {}
    ...
    def load_env_local():
        ...
        for line in content.splitlines():
            ...
            IpProxyDetectionTestRunner._env[key] = val

    @staticmethod
    def getenv(key):
        val = IpProxyDetectionTestRunner._env.get(key)
        if val is not None:
            return val
    ...
    @staticmethod
    def env_override(m):
        live = IpProxyDetectionTestRunner.getenv("IP_PROXY_DETECTION_TEST_LIVE")
        override = IpProxyDetectionTestRunner.getenv("IP_PROXY_DETECTION_TEST_OVERRIDE")

        if live == "TRUE" or override == "TRUE":
            for key in list(m.keys()):
                envval = IpProxyDetectionTestRunner.getenv(key)
        ...
        explain = IpProxyDetectionTestRunner.getenv("IP_PROXY_DETECTION_TEST_EXPLAIN")
        if explain is not None and explain != "":
            m["IP_PROXY_DETECTION_TEST_EXPLAIN"] = explain
    ...
    def load_test_control():
        ...
        """
        if IpProxyDetectionTestRunner._test_control is not None:
            return IpProxyDetectionTestRunner._test_control
        ctrl_path = os.path.join(os.path.dirname(__file__), "sdk-test-control.json")
        try:
            with open(ctrl_path, "r") as f:
                IpProxyDetectionTestRunner._test_control = json.load(f)
        except (FileNotFoundError, IOError, ValueError):
            IpProxyDetectionTestRunner._test_control = {
                "version": 1,
                "test": {"skip": {
    ...
    def is_control_skipped(kind, name, mode):
        ...
        ctrl = IpProxyDetectionTestRunner.load_test_control()
...
def load_env_local():
    IpProxyDetectionTestRunner.load_env_local()


def env_override(m):
    return IpProxyDetectionTestRunner.env_override(m)


def entity_data(v):
    return IpProxyDetectionTestRunner.entity_data(v)


def entity_list_to_data(lst):
    return IpProxyDetectionTestRunner.entity_list_to_data(lst)


def is_control_skipped(kind, name, mode):
    return IpProxyDetectionTestRunner.is_control_skipped(kind, name, mode)


def load_test_control():
    return IpProxyDetectionTestRunner.load_test_control()


def live_client_options():
    return IpProxyDetectionTestRunner.live_client_options()


def live_delay_ms():
    return IpProxyDetectionTestRunner.live_delay_ms()
