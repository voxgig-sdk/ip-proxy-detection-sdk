...
require_relative 'IpProxyDetection_types'
...
class IpProxyDetectionSDK
  ...
  def initialize(options = {})
    ...
    feature_opts = IpProxyDetectionHelpers.to_map(VoxgigStruct.getprop(@options, "feature"))
    if feature_opts
      featureorder = VoxgigStruct.getpath(@options, "__derived__.featureorder")
      if featureorder.is_a?(Array)
        featureorder.each do |fname|
          fopts = IpProxyDetectionHelpers.to_map(feature_opts[fname])
          if fopts && fopts["active"] == true
            utility.feature_add.call(@_rootctx, IpProxyDetectionFeatures.make_feature(fname))
          end
        end
      end
    end
  ...
  def get_utility
    IpProxyDetectionUtility.copy(@_utility)
  end
  ...
  def prepare(fetchargs = {})
    ...
    ctrl = IpProxyDetectionHelpers.to_map(VoxgigStruct.getprop(fetchargs, "ctrl")) || {}
    ...
    params = IpProxyDetectionHelpers.to_map(VoxgigStruct.getprop(fetchargs, "params")) || {}
    query = IpProxyDetectionHelpers.to_map(VoxgigStruct.getprop(fetchargs, "query")) || {}
  ...
  def op_denied(op)
    ...
    {
      "ok" => false,
      "err" => IpProxyDetectionError.new(
        "#{op}_allow",
        "IpProxyDetectionSDK: #{op}: operation not allowed by" \
  ...
  def raw_request(fetchargs = {})
    ...
    rescue IpProxyDetectionError => err
    ...
    if fetched.is_a?(Hash)
      status = IpProxyDetectionHelpers.to_int(VoxgigStruct.getprop(fetched, "status"))
  ...
  def graphql(query, variables = nil, ctrl = nil)
    ...
    if errors.is_a?(Array) && !errors.empty?
      ...
      res["ok"] = false
      res["err"] = IpProxyDetectionError.new(
        "graphql_error", "IpProxyDetectionSDK: graphql: #{msg}")
      res["graphql"] = errors
    end
  ...
  def self.test(testopts = nil, sdkopts = nil)
    ...
    sdk = IpProxyDetectionSDK.new(sdkopts)
    sdk.mode = "test"
    sdk
  end
end
