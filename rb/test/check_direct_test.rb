...
class CheckDirectTest < Minitest::Test
  def test_direct_load_check
    setup = check_direct_setup({ "id" => "direct01" })
    _should_skip, _reason = Runner.is_control_skipped("direct", "direct-load-check", setup[:live] ? "live" : "unit")
    if _should_skip
...
def check_direct_setup(mockres)
