...
class CheckEntity
  ...
  def make
    opts = @_entopts.dup
    CheckEntity.new(@_client, opts)
  end

  def data_set(args)
    if args
      @_data = IpProxyDetectionHelpers.to_map(VoxgigStruct.clone(args)) || {}
      @_utility.feature_hook.call(@_entctx, "SetData")
    end
  end

  # @return [Check, Hash] the current Check data
  def data_get
  ...
  def match_set(args)
    if args
      @_match = IpProxyDetectionHelpers.to_map(VoxgigStruct.clone(args)) || {}
      @_utility.feature_hook.call(@_entctx, "SetMatch")
    end
  end

  # @return [Hash] the current match filter (any subset of Check fields)
  def match_get
  ...
  # Load a single Check.
  #
  # @param reqmatch [CheckLoadMatch, Hash, nil] match criteria (id/query fields);
  #   optional — an entity with no id-like key loads with no match (nil is treated
  #   as an empty match, so client.Check.load works with no args).
  # @param ctrl [Object, nil] optional per-call control
  # @return [Check, Hash] the loaded Check; raises IpProxyDetectionError on failure
    ...
    _run_op(ctx) do
      if ctx.result
        ...
        if ctx.result.resdata
          @_data = IpProxyDetectionHelpers.to_map(VoxgigStruct.clone(ctx.result.resdata)) || {}
        end
      end
    end
  end
