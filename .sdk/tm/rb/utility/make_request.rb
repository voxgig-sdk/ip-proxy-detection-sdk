...
module IpProxyDetectionUtilities
  MakeRequest = ->(ctx) {
    ...
    spec = ctx.spec
    utility = ctx.utility
    response = IpProxyDetectionResponse.new({})
    ...
    if fetch_err
      ...
      response = IpProxyDetectionResponse.new({ "err" => ctx.make_error("request_no_response", "response: undefined") })
    elsif fetched.is_a?(Hash)
      response = IpProxyDetectionResponse.new(fetched)
    else
