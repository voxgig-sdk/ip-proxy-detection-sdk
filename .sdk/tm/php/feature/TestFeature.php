...
// IpProxyDetection SDK test feature

require_once __DIR__ . '/BaseFeature.php';
require_once __DIR__ . '/../utility/Param.php';

class IpProxyDetectionTestFeature extends IpProxyDetectionBaseFeature
{
    ...
    public function init(IpProxyDetectionContext $ctx, array $options): void
    {
        ...
        $test_fetcher = function (IpProxyDetectionContext $fctx, string $_fullurl, array $_fetchdef) use ($entity): array {
            ...
            if ($op->name === 'load') {
                ...
                $id = IpProxyDetectionParam::call($fctx, 'id');
    ...
    public function makeNetsim(array $net, callable $inner): callable
    {
        ...
        return function (IpProxyDetectionContext $fctx, string $url, array $fetchdef) use ($net, $inner, $pick_latency, $sleep): array {
            $this->_netcalls++;
            $call = $this->_netcalls;
     ...
     */
    public function buildArgs(IpProxyDetectionContext $ctx, $op, $args): array
    {
        ...
        foreach ($keys as $k) {
            ...
            if ($is_id || $in_reqd) {
                $v = IpProxyDetectionParam::call($ctx, $k);
