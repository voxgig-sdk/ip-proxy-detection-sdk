...
from projectname_sdk.core.control import IpProxyDetectionControl
...
class IpProxyDetectionContext:
    def __init__(self, ctxmap=None, basectx=None):
        ...
        # Ctrl
        self.ctrl = IpProxyDetectionControl()
        ...
        if isinstance(sp, IpProxyDetectionSpec):
            self.spec = sp
        elif basectx is not None:
        ...
        if isinstance(r, IpProxyDetectionResult):
            self.result = r
        elif basectx is not None:
        ...
        if isinstance(rp, IpProxyDetectionResponse):
            self.response = rp
        elif basectx is not None:
    ...
    def resolve_op(self, opname):
        ...
        if opname == "":
            return IpProxyDetectionOperation({})
    ...
    def make_error(self, code, msg):
        return IpProxyDetectionError(code, msg, self)
